      ...
      <Masonry breakpointCols={breakpointCols} className="masonry-grid" columnClassName="masonry-grid-col">
        {galleryImages.map((img, i) => (
          <div key={img.id} onClick={() => setLightbox(i)}
            style={{ cursor: 'pointer', position: 'relative', overflow: 'hidden', borderRadius: 10 }}>
            ...
            />
            <div
              style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, rgba(0,0,0,0.75) 0%, transparent 50%)', opacity: 0, transition: 'opacity 0.25s', display: 'flex', alignItems: 'flex-end', padding: 14, borderRadius: 10 }}
