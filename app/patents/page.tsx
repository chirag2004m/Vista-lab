          ...
          return (
            <div key={patent.id} style={{ padding: '32px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10, flexWrap: 'wrap' }}>
                ...
                <span style={{ fontSize: 10, padding: '2px 10px', borderRadius: 100, fontFamily: 'Space Mono, monospace', fontWeight: 700, background: isGranted ? 'rgba(74,222,128,0.08)' : 'rgba(251,191,36,0.08)', border: `1px solid ${isGranted ? 'rgba(74,222,128,0.25)' : 'rgba(251,191,36,0.25)'}`, color: isGranted ? '#4ade80' : '#fbbf24' }}>{patent.status}</span>
              ...
              <div style={{ display: 'flex', gap: 24, flexWrap: 'wrap' }}>
                <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.3)', fontFamily: 'Space Mono, monospace' }}>Filed: {new Date(patent.filedDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>
                {patent.grantDate && <span style={{ fontSize: 12, color: '#4ade80', fontFamily: 'Space Mono, monospace' }}>Granted: {new Date(patent.grantDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}</span>}
              </div>
            </div>
          );
        })}
