    ...
    <div style={{ maxWidth: 1400, margin: '0 auto', padding: '72px 40px 60px' }}>
      <div style={{ marginBottom: 48 }}>
        ...
        <p style={{ fontSize: 15, color: 'var(--text-secondary)', maxWidth: 560 }}>
          A diverse group of researchers united by curiosity and a passion for advancing intelligent systems.
        </p>
      </div>
...
function MemberCard({ member }: { member: typeof teamMembers[0] }) {
  ...
  return (
    <div
      style={{ background: '#0d1117', border: '1px solid #1e3a5f', borderRadius: 14, overflow: 'hidden', display: 'flex', flexDirection: 'column', transition: 'border-color 0.25s, box-shadow 0.25s, transform 0.25s' }}
    ...
    >
      ...
      <div style={{ padding: '14px 16px 0', flex: 1, display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ marginBottom: 6 }}>
          ...
          <a href={`mailto:${member.email}`} style={{ fontSize: 11, color: 'rgba(255,255,255,0.4)', textDecoration: 'none' }}>
      ...
      <div style={{ padding: '10px 16px 14px', borderTop: '1px solid rgba(255,255,255,0.05)', display: 'flex', gap: 8, marginTop: 'auto' }}>
        {[
          { icon: <Linkedin size={15} />, href: member.linkedin, label: 'LinkedIn' },
          { icon: <Github size={15} />, href: '#', label: 'GitHub' },
          { icon: <BookOpen size={15} />, href: member.googleScholar, label: 'Scholar' },
          ...(member.role === 'Principal Investigator' ? [{ icon: <ExternalLink size={15} />, href: '#', label: 'Website' }] : []),
        ].map(({ icon, href, label }) => (
