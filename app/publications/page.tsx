        ...
        {filtered.map(pub => (
          <div key={pub.id} style={{ padding: '32px 0', borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 10 }}>
              <span style={{ fontSize: 11, fontFamily: 'Space Mono, monospace', fontWeight: 700, color: pub.type === 'Journal' ? '#a78bfa' : 'var(--accent)', letterSpacing: '0.05em' }}>{pub.venue}</span>
              <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.2)', fontFamily: 'Space Mono, monospace' }}>{pub.year}</span>
              <span style={{ fontSize: 10, padding: '2px 8px', borderRadius: 4, background: 'rgba(255,255,255,0.05)', color: 'rgba(255,255,255,0.3)', fontFamily: 'Space Mono, monospace' }}>{pub.type}</span>
            </div>
            <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 18, color: '#e8edf2', lineHeight: 1.3, marginBottom: 8 }}>{pub.title}</h3>
            <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.4)', marginBottom: 12, fontStyle: 'italic' }}>{pub.authors}</p>
