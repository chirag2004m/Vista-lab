...
export default function ResearchPage() {
  return (
    <div style={{ maxWidth: 1400, margin: '0 auto', padding: '72px 40px 60px' }}>
      <div style={{ marginBottom: 48 }}>
        ...
        <p style={{ fontSize: 15, color: 'var(--text-secondary)', maxWidth: 560, lineHeight: 1.6 }}>
          Fundamental and applied research across machine learning, distributed systems, and AI for science.
        </p>
      </div>
...
function ProjectCard({ project }: { project: typeof researchProjects[0] }) {
  ...
  return (
    <div
      className="group"
      style={{ background: 'var(--bg-card)', border: '1px solid var(--border)', borderRadius: 16, overflow: 'hidden', display: 'flex', flexDirection: 'column', transition: 'border-color 0.25s, transform 0.25s, box-shadow 0.25s' }}
      onMouseEnter={e => { const el = e.currentTarget as HTMLElement; el.style.borderColor = 'var(--border-bright)'; el.style.transform = 'translateY(-4px)'; el.style.boxShadow = '0 16px 48px rgba(0,0,0,0.45)'; }}
    ...
    >
      <div className="relative overflow-hidden" style={{ height: 170, flexShrink: 0 }}>
        <img src={project.image} alt={project.title} className="group-hover:scale-105"
        ...
        <span style={{
          position: 'absolute', top: 10, right: 10, padding: '3px 10px', borderRadius: 100,
          fontSize: '0.62rem', fontFamily: 'Space Mono, monospace', fontWeight: 600, backdropFilter: 'blur(8px)',
          background: isActive ? 'rgba(0,212,255,0.18)' : 'rgba(255,255,255,0.07)',
          border: `1px solid ${isActive ? 'rgba(0,212,255,0.5)' : 'rgba(255,255,255,0.12)'}`,
      ...
      <div style={{ padding: '16px 18px 18px', display: 'flex', flexDirection: 'column', flex: 1 }}>
        ...
        <h3 style={{ fontFamily: 'Syne, sans-serif', fontWeight: 700, fontSize: 14, lineHeight: 1.4, marginBottom: 8 }}>{project.title}</h3>
